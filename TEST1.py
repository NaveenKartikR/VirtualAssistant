...
import string, random, json, pickle
import numpy as np
import nltk
...
def speak_text_cmd(text, language = 'en'):
    if language == 'en':
        va_Laura.say(text)
        va_Laura.runAndWait()
    else:
...
def ambient_noise_adjust(adjust_time = 1):
    with mic:
        recog.adjust_for_ambient_noise(mic, duration = adjust_time)
...
def check_everything():
    ...
    found = False
    speak_text_cmd("Say something to test mic")
    while not found:
        try:
            with mic:
                try:
            ...
            speak_text_cmd("Mic access denied. Give access to use Virtual Assistant")
            exit()
    found = False
    isfirstwarning = True
    while not found:
        try:
            ...
            if isfirstwarning:
                print("\nNo internet connection. Active internet connection is required to use Virtual Assistant\n")
                speak_text_cmd("No internet connection. Active internet connection is required to use Virtual Assistant")
...
def translate():
    ...
    try:
        ...
        speak_text_cmd(translate.text, dest_code)
...
def predict_class(sentence, language = 'en'):
    if language != 'en':
        translate_dict = translate_client.translate(sentence, dest = "en", src = language)
        sentence = translate_dict.text
...
def get_response(intents_list, intents_json, language = 'en'):
    ...
    for i in list_of_intents:
        if i['tag'] == tag:
            ...
            break
    if (language != "en"):
        translate_dict = translate_client.translate(result, dest = language, src = "en")
...
def run_VA():
    ...
    while True:
        ambient_noise_adjust()
        print("\n YOU  : ", end = "")
        message = listen(lang = dest_code)
        ints = predict_class(message, dest_code)
        if ints == []:
            print("LAURA : Sorry I don't understand your statement. Can you please repeat?")
            speak_text_cmd("Sorry I don't understand your statement. Can you please repeat?", dest_code)
        else:
            res, tag = get_response(ints, intents, dest_code)
            print("LAURA :", res)
            speak_text_cmd(res, dest_code)
            if tag == "goodbye":
                print("\n\n--------------------------------------------------------------------------------------------------------------------\n\n")
                exit()
            reply(tag)
